...
ns = Namespace(  # pylint: disable=invalid-name
    "tests/", descriptions="Test aggregations."
)
...
@ns.param(
    "test_status", "The status of the test, currently only able to be set to failed."
)
class AggregatedTests(Resource):
    ...
    def get(self, job_name=None):
        ...
        parser.add_argument(
            ...
            choices=["failed"],
            default=None,
            help='Test status must be "failed"',
        )
        parser.add_argument(
            "start",
        ...
        match_status = Q(
            "match", br_status_key=BuildResults.BuildStatus.FAILURE.name
        ) | Q("match", br_status_key=BuildResults.BuildStatus.UNSTABLE.name)
        range_time = Q(
        ...
        # Setup aggregation
        test_agg = A(
            "terms", field="br_tests_object.br_tests_failed_object.br_fullname.raw"
        )
